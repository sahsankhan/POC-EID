...
And("I fill in 'Add a Shift' modal with the following data:", (dataTable) => {
    ...
    if (data.hasOwnProperty('start time')) {
        SchedulingPage.selectStartTimeLocator().type('{selectall}{backspace}')
        SchedulingPage.selectStartTimeLocator().type(data["start time"])
    }
    if (data.hasOwnProperty('end time')) {
        SchedulingPage.selectEndTimeLocator().type('{selectall}{backspace}')
        SchedulingPage.selectEndTimeLocator().type(data["end time"])
    }
...
Then("I click shift card with employee name {string} and shift time {string}", (employee, shift) => {
    SchedulingPage.shiftCardLocator(employee, shift).should("be.visible").click();
});
